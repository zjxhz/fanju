...
- (UIView *)keyView {
	...
	if (w.subviews.count > 0) {
        UIView* keyView = [w.subviews lastObject];
        if(keyView.hidden && w.subviews.count > 1) {
            keyView  = [w.subviews objectAtIndex:w.subviews.count -2 ];//in case OverlayViewController is not visible
        }
		return keyView;
	} else {
		return w;
